...
export default {
  ...
  computed: {
    ...
    hasDevices() {
      return this.devices.some((d) => d.name || d.watts || d.hours || d.qty)
    },
  },
  ...
  methods: {
    ...
    tabClass(modeName) {
      return [
        ...
        this.mode === modeName ? 'bg-blue-500 text-white' : 'bg-gray-200',
      ]
    },
  },
  mounted() {
    ...
    if (systemData.load) {
      this.sunHours = systemData.load.sunHours || 4

      this.monthlyKwh = systemData.load.monthlyKwh || null

      this.mode = systemData.load.mode || 'direct'

      this.bill = systemData.load.bill || null

      this.rate = systemData.load.rate || null

      this.devices = systemData.load.devices || []
    }
  },
  watch: {
    $data: {
...
<template>
  <div class="p-6 mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-2 mb-6">
      <button @click="setMode('direct')" :class="tabClass('direct')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          width="20"
      ...
      <button @click="setMode('bill')" :class="tabClass('bill')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          width="20"
      ...
      <button @click="setMode('device')" :class="tabClass('device')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          width="20"
    ...
    <div v-if="mode === 'direct'" class="grid md:grid-cols-2 gap-4">
      ...
      <div class="bg-white border rounded-lg p-4 shadow-sm">
        <p class="text-sm text-gray-500 mb-1">Monthly Consumption</p>
        ...
        <p class="text-xs text-gray-400 mt-2">Check your electric bill for this value</p>
      </div>

      <!-- Sun Hours -->
      <div class="bg-white border rounded-lg p-4 shadow-sm">
        <p class="text-sm text-gray-500 mb-1">Sun Peak Hours</p>

        <div class="flex items-center justify-between border rounded-md px-3 py-2">
          <input type="number" step="0.1" class="outline-none w-full text-lg" v-model="sunHours" />
          <span class="text-gray-500 ml-2">hours/day</span>
        </div>

        <p class="text-xs text-gray-400 mt-2">Philippines average: 4–5 hours</p>
      </div>
    </div>
    ...
    <div v-if="mode === 'bill'" class="mt-6">
      <div class="grid md:grid-cols-3 gap-4">
        <div class="bg-white border rounded-lg p-4 shadow-sm">
          <p class="text-sm text-gray-500 mb-1">Monthly Electric Bill</p>
          ...
          <p class="text-xs text-gray-400 mt-2">Enter your total monthly electric bill</p>
        </div>

        <div class="bg-white border rounded-lg p-4 shadow-sm">
          <p class="text-sm text-gray-500 mb-1">Rate per KWh</p>
        ...
        <div class="bg-white border rounded-lg p-4 shadow-sm">
          ...
          <p class="text-xs text-gray-400 mt-2">Philippines avverage: 4-5 hours</p>
        </div>
      </div>
      ...
      <div>
        <div>
          <p
            class="mt-3 text-sm text-green-600 font-semibold flex items-center justify-start gap-2 bg-green-200 p-1 rounded-lg"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
    ...
    <div v-if="mode === 'device'">
      <div class="mb-4">
        ...
        <input type="number" v-model="sunHours" class="border rounded px-3 py-2 w-full" />
      ...
      <div class="bg-white p-2 rounded shadow">
        <div class="flex items-center justify-between mb-2">
          ...
          <button
            @click="addDevice"
            class="flex items-center gap-1 text-blue-600 bg-blue-200 p-1 rounded-md"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
            ...
            <div class="grid grid-cols-6 gap-2 mb-2">
              ...
              <button @click="removeDevice(index)">
                <svg
                  ...
                  width="25"
                  height="25"
                  fill="rgba(231,18,18,1)"
                >
                  <path
                    d="M17 6H22V8H20V21C20 21.5523 19.5523 22 19 22H5C4.44772 22 4 21.5523 4 21V8H2V6H7V3C7 2.44772 7.44772 2 8 2H16C16.5523 2 17 2.44772 17 3V6ZM18 8H6V20H18V8Z"
                  ></path>
                </svg>
              </button>
            </div>
      ...
      <template v-if="mode === 'direct'">
        <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold">{{ dailyWh.toFixed(0) }} Wh</p>
          </div>
        ...
        <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm">Daily Consumption</p>
            <p class="text-xl font-bold">{{ dailyKwh.toFixed(2) }} kWh</p>
          </div>
          <div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold">{{ hourlyWatts.toFixed(0) }} W</p>
          </div>
          <div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
      ...
      <template v-else-if="mode === 'bill'">
        <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold">{{ computedMonthlyKwh.toFixed(1) }} kWh</p>
          </div>
        ...
        <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
          <div>
            ...
            <p class="text-xl font-bold">{{ requiredSolar.toFixed(2) }} kW</p>
          </div>
    ...
    <div class="hidden md:block py-4 text-sm text-blue-600">
      ...
      <template v-if="mode === 'direct'">
        <div class="flex items-center justify-start gap-2 font-semibold bg-blue-100 p-1 rounded-lg">
          ...
          Formula: Daily Wh = ({{ computedMonthlyKwh.toFixed(0) }} kWh / 30 days) × 1000 =
      ...
      <template v-else-if="mode === 'bill'">
        <div class="flex items-center justify-start gap-2 font-semibold bg-blue-100 p-1 rounded-lg">
          ...
          Formula: (₱{{ bill }} / ₱{{ rate }} per kWh / 30 days / {{ sunHours }}h sun peak) =
    ...
    <div class="md:hidden block py-4 text-sm text-blue-600">
      ...
      <template v-if="mode === 'direct'">
        <div
          class="flex flex-col md:flex-row md:items-center gap-2 font-semibold bg-blue-100 p-3 rounded-lg break-words"
        >
      ...
      <template v-else-if="mode === 'bill'">
        ...
        >
          ...
          <span>
            ...
            <span class="font-bold"> {{ requiredSolar.toFixed(2) }} kW </span>
