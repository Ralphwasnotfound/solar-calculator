...
export default {
  ...
  computed: {
    ...
    requiredCapacity() {
      if (!this.selectedBattery || !this.dailyWh || !this.selectedInverter) {
      ...
      return Math.round(
        (this.autonomyType === 'hours'
          ...
          (this.dod * parseInt(this.selectedInverter?.voltage)),
      )
    },
    ...
    batteriesNeeded() {
      ...
      const computed = Math.ceil(this.requiredCapacity / this.selectedBattery.capacityAh)
  ...
  mounted() {
    ...
    if (systemData.battery) {
      ...
      this._savedBatteriesNeeded = saved.batteriesNeeded || 0

      this._savedRequiredCapacity = saved.requiredCapacity || 0
    }

    if (systemData.battery) {
      const saved = systemData.battery.selectedBattery

      if (saved) {
        this.selectedBatteryId = saved.energyWh
      }

      this.autonomyDays = systemData.battery.autonomyDays || 1

      this.autonomyValue = systemData.battery.autonomyValue || 12

      this.autonomyType = systemData.battery.autonomyType || 'hours'

      this.dod = systemData.battery.dod || 0.9
    }
  },
  watch: {
    selectedBatteryId(id) {
      const battery = this.batteries.find((b) => b.energyWh === id)
  ...
  <div class="p-6 mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
      <label class="relative inline-flex items-center cursor-pointer">
        ...
        <div
          class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-blue-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"
        ></div>
      </label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <!-- BATTERY SELECT -->
      <div>
        <label class="block text-sm text-gray-500 mb-2"> Select Battery </label>

        <select v-model="selectedBatteryId" class="w-full border rounded-lg px-4 py-3">
          <option :value="null">Select Battery</option>

          <option v-for="battery in batteries" :key="battery.energyWh" :value="battery.energyWh">
            {{ battery.brand }}

            {{ battery.voltage }}V

            {{ battery.capacityAh }}Ah ({{ battery.energyWh }}Wh)
          </option>
        </select>
      </div>
      ...
      <div>
        <label class="block text-sm text-gray-500 mb-2"> Autonomy </label>
    ...
    <div v-if="selectedBattery" class="border rounded-lg p-5 mb-6">
      ...
      <div class="space-y-4 text-sm">
        ...
        <div class="flex justify-between border-b pb-2">
          ...
          <span class="font-medium"> {{ selectedBattery.voltage }}V </span>
        </div>
        ...
        <div class="flex justify-between border-b pb-2">
          ...
          <span class="font-medium">
            {{ selectedBattery.capacityAh }}Ah ({{ selectedBattery.energyWh }}Wh)
          </span>
        </div>

        <!-- DOD -->
        <div class="flex justify-between">
          <span class="text-gray-500"> DOD </span>
    ...
    <div v-if="!selectedBattery" class="border-2 border-gray-300 rounded-xl p-10 text-center mb-6">
      ...
      <p class="text-gray-400">
        Select a battery to calculate required capacity, bank voltage, and battery configuration.
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div class="bg-blue-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">System Voltage</p>
      ...
      <div class="bg-orange-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">Required Capacity</p>

          <h3 class="text-3xl font-bold text-orange-600">{{ requiredCapacity }} Ah</h3>
        </div>
      ...
      <div class="bg-blue-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">Batteries Needed</p>

          <h3 class="text-3xl font-bold text-blue-700">{{ batteriesNeeded }} pcs</h3>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div class="bg-blue-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">Bank Voltage</p>
      ...
      <div class="bg-cyan-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">Total Capacity</p>
      ...
      <div class="bg-green-50 p-4 rounded-lg flex items-center justify-between">
        <div>
          <p class="text-sm text-gray-500 mb-2">Total Energy</p>
    ...
    <div v-if="selectedBattery" class="bg-blue-100 text-blue-700 p-2 rounded-lg text-sm">
      <p class="font-semibold flex items-center gap-2">
        ...
        Battery Configuration: 1S{{ batteriesNeeded }}P (1 in series × {{ batteriesNeeded }} in
        parallel) = {{ batteriesNeeded }} batteries total
      </p>
    </div>
    ...
    <div class="hidden md:block pt-2">
      <div class="bg-blue-100 text-blue-700 p-2 rounded-lg text-sm">
        <p class="font-semibold flex items-center gap-2">
          ...
          Formula: Capacity = ({{ Math.round(dailyWh) }}Wh × {{ autonomyValue }}
          {{ autonomyType === 'days' ? 'd' : 'hrs' }}) / ({{ dod }} DOD ×
          {{ selectedInverter?.voltage?.replace('V', ' V') }}) = {{ requiredCapacity }} Ah
        </p>
      </div>
    </div>

    <div class="md:hidden block pt-2">
      <div
        class="bg-blue-100 text-blue-700 p-3 rounded-lg text-sm flex flex-col md:flex-row gap-2 break-words"
      >
        ...
        <span>
          Capacity = ({{ Math.round(dailyWh) }}Wh ×
          {{ autonomyValue }}
          {{ autonomyType === 'days' ? 'd' : 'hrs' }}) / ({{ dod }} DOD ×
          {{ selectedInverter?.voltage?.replace('V', ' V') }}) =
