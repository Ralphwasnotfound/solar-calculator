...
export default {
  props: ['selectedPanel', 'selectedInverter', 'totalPanels'],
  ...
  computed: {
    maxSeriesPanels() {
      ...
      return Math.floor(
        parseFloat(this.selectedInverter.maxVoltagePerMppt) / parseFloat(this.selectedPanel.voc),
      )
    },
    ...
    currentStatus() {
      ...
      const stringCurrent = this.mppts[0].parallel * parseFloat(this.selectedPanel.isc)

      const maxCurrent = parseFloat(this.selectedInverter.maxCurrentPerMppt)
    ...
    mpptStatus() {
      return this.mppts.map((mppt) => {
        const voltage = mppt.series * (this.selectedPanel?.voc || 0)

        const current = mppt.parallel * (this.selectedPanel?.isc || 0)

        const maxVoltage = parseFloat(this.selectedInverter?.maxVoltagePerMppt || 0)

        const maxCurrent = parseFloat(this.selectedInverter?.maxCurrentPerMppt || 0)

        return {
          voltageSafe: voltage <= maxVoltage,

          currentSafe: current <= maxCurrent,
        }
      })
    },
  },
  ...
  watch: {
    ...
    mppts: {
      handler(newValue) {
        const totalSeriesVoltage = Math.max(...newValue.map((mppt) => mppt.series))

        const maxParallelStrings = Math.max(...newValue.map((mppt) => mppt.parallel))
        ...
        systemData.solar = {
          ...
          seriesPanels: totalSeriesVoltage,

          parallelStrings: maxParallelStrings,
        }
        ...
        this.$emit('update-series-panels', totalSeriesVoltage)

        this.$emit('update-parallel-strings', maxParallelStrings)
      },
  ...
  <div class="p-6 mx-auto">
    ...
    >
      ...
      <span v-else class="flex items-center gap-2">
        ...
        >
          ...
          ></path></svg
        >MPPT 1: Isc exceeds max {{ currentStatus.max }}A
      </span>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold">{{ maxSeriesPanels }} pcs</p>
        </div>
      ...
      <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold">{{ totalPanels }} pcs</p>
        </div>
      ...
      <div class="bg-cyan-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold">{{ Number(currentStatus.max).toFixed(1) }} A</p>
        </div>
    ...
    <div class="border rounded-lg p-4">
      <div class="flex md:flex-row flex-col justify-between mb-4">
        ...
        <button class="bg-gray-200 px-3 py-1 rounded">Reset</button>
      </div>
      ...
      <div class="bg-blue-100 text-blue-700 p-2 rounded-lg mb-3 text-sm flex items-start gap-2">
        ...
        <p class="leading-relaxed">
          Click on an MPPT card to adjust the number of panels in series and parallel strings. Each
          <span class="font-semibold">MPPT</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          ...
          :key="index"
          @click.stop="activeMppt = activeMppt === index ? null : index"
          :class="
            mppt.series > 0
              ? mpptStatus[index]?.voltageSafe && mpptStatus[index]?.currentSafe
                ? 'border-green-300'
        ...
        >
          <div class="text-center mb-4">
            <p class="text-sm text-gray-500">MPPT {{ index + 1 }}</p>

            <p class="text-3xl font-bold">
              {{ mppt.series * mppt.parallel }}
            </p>

            <p class="text-sm text-gray-500">panels</p>

            <div class="mt-3 text-sm">
              <p>{{ mppt.series }}S × {{ mppt.parallel }}P</p>

              <p>{{ (mppt.series * mppt.parallel * (selectedPanel?.watt || 0)).toFixed(0) }}W</p>

              <p>Voc:{{ (mppt.series * (selectedPanel?.voc || 0)).toFixed(1) }} V</p>

              <p>Isc:{{ (mppt.parallel * (selectedPanel?.isc || 0)).toFixed(1) }} A</p>
            </div>
            ...
            >
              <span class="flex items-center gap-2 justify-center">
                ...
                MPPT {{ index + 1 }}: Isc ({{
                  (mppt.parallel * (selectedPanel?.isc || 0)).toFixed(1)
                }}A) exceeds max {{ Number(currentStatus.max).toFixed(1) }}A
              </span>
            </div>
          </div>
          ...
          <div v-if="activeMppt === index">
            ...
            <div class="space-y-3">
              <div>
                <label class="text-sm text-gray-500"> Series Panels </label>
                <div class="flex gap-2">
                  ...
                  <div
                    class="w-24 bg-gray-100 rounded flex items-center justify-center text-sm text-gray-600"
                  >
              ...
              <div>
                <label class="text-sm text-gray-500"> Parallel Strings </label>
                <div class="flex gap-2">
                  <input
                    @click.stop
                    type="number"
      ...
      <div class="flex items-start gap-2">
        ...
        <div>
          <div class="font-semibold">
            <h1>Max Series/MPPT:</h1>

            {{ selectedInverter?.maxVoltagePerMppt }}V (max MPPT) / {{ selectedPanel?.voc }}V (Voc)
            =
            {{ maxSeriesPanels }}
            panels
          </div>
