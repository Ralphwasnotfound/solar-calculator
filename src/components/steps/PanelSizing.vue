...
import PanelSearchModal from '../modals/PanelSearchModal.vue'
import PanelSellers from '../modals/PanelSellers.vue'
...
export default {
  ...
  props: ['requiredSolar', 'dailyWh', 'sunHours'], // from LoadAnalysis
  ...
  methods: {
    async loadPanels() {
      ...
      this.panelsError = ''
      try {
        const querySnapShot = await getDocs(collection(db, 'solarPanels'))
        this.panels = querySnapShot.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
        }))
      } catch (error) {
        ...
        const code = safeCodes.includes(error?.code) ? error.code : 'unknown'
        console.error('Failed to load solarPanels', { code })
        this.panelsError = 'Unable to load solar panels. Please check your connection and reload the page.'
      } finally {
    ...
    searchPanel() {
      ...
      this.results = this.panels.filter((p) =>
        (p.brand + ' ' + p.model).toLowerCase().includes(this.search.toLowerCase()),
      )
  ...
  async mounted() {
    ...
    if (systemData.solar) {
      this.mode = systemData.solar.mode || 'list'

      this.search = systemData.solar.search || ''

      this.selectedPanel = systemData.solar.selectedPanel || null

      this.useSafety = systemData.solar.useSafety ?? true
    }
    // Restore the saved selection before the independent legacy dropdown load.
...
<template>
  <div class="p-6 mx-auto">
    ...
    <div>
      <div
        class="flex flex-col md:flex-row items-center justify-between gap-2 py-2 w-full text-md rounded font-semibold mb-6"
      >
        <button
          @click="mode = 'list'"
          :class="
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      ...
      <div v-if="mode === 'list'" class="mb-4 relative">
        ...
        >
          ...
          >
            <p class="font-medium">{{ panel.brand }} {{ panel.model }}</p>
            <p class="text-xs text-gray-500">{{ panel.watt }}W • {{ panel.efficiency }}%</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mt-4">
      ...
      <template v-if="mode === 'list'">
        ...
        <div v-if="selectedPanel" class="border rounded-lg p-4 bg-gray-50">
          ...
          <PanelSellers v-if="selectedPanel.technicalSource === 'CEC' && selectedPanel.specificationsVerified === true" :key="selectedPanel.id" :panel="selectedPanel" />
        </div>
        ...
        <div v-else class="p-6 border rounded-lg text-center text-gray-400">
          <p class="text-lg flex items-center justify-center gap-2 mb-2">
            ...
            No Solar Panel selected
          </p>
          <p class="text-sm">Search and select a panel to view specifications</p>
        </div>
      </template>
    ...
    <label class="flex items-center gap-3 mb-2 cursor-pointer mt-4">
      <!-- TOGGLE -->
      <div class="relative">
        <input type="checkbox" v-model="useSafety" class="sr-only peer" />
      ...
      <!-- TEXT (RIGHT SIDE) -->
      <span class="text-sm font-medium text-gray-700"> Safety Factor (1.25) </span>
    </label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
      ...
      <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-xl font-bold">{{ adjustedPower.toFixed(0) }} W</p>
        </div>
      ...
      <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-xl font-bold">{{ panelsNeeded }} pcs</p>
        </div>
      ...
      <div class="bg-blue-50 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-xl font-bold">{{ totalPvPower }} W</p>
        </div>
    ...
    <div class="hidden md:block py-4 text-sm text-blue-600">
      <div class="flex items-center justify-start gap-2 font-semibold bg-blue-100 p-1 rounded-lg">
        ...
        <span class="flex items-center gap-1">
          ...
          PV Power = ({{ (dailyWh || 0).toFixed(0) }} Wh / {{ sunHours || 4 }}h) =
          {{ ((dailyWh || 0) / (sunHours || 4)).toFixed(0) }} W | Panels =
