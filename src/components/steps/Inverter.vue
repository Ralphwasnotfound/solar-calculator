...
import InverterSearchModal from '../modals/InverterSearchModal.vue';
...
export default {
  ...
  computed: {
    filteredInverters() {
      return this.inverterList.filter((inv) => {
        return (
          inv.type === 'hybrid' &&
          (inv.brand + ' ' + inv.model).toLowerCase().includes(this.searchQuery.toLowerCase())
        )
      })
    },
    ...
    recommendedInverter() {
      return this.filteredInverters.find((inv) => inv.power >= this.requiredInverterPower)
    },
  ...
  async mounted() {
    ...
    if (systemData.inverter) {
      ...
      this.searchQuery = systemData.inverter.searchQuery || ''

      this.useCustom = systemData.inverter.useCustom || false

      this.inverterType = systemData.inverter.inverterType || 'hybrid'
    }
  },
  ...
  methods: {
    async loadInverters() {
      const querySnapShot = await getDocs(collection(db, 'solarInverters'))

      this.inverterList = querySnapShot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }))
    },
    ...
    onSearch() {
      ...
      const match = this.filteredInverters.find(
        (inv) => `${inv.brand} ${inv.model}`.toLowerCase() === this.searchQuery.toLowerCase(),
      )
    ...
    $data: {
      handler() {
        ...
        systemData.inverter = {
          ...
          requiredInverterPower: this.requiredInverterPower,
        }
...
<template>
  <div class="p-6 mx-auto">
    ...
    <!-- RECOMMENDATION -->
    <div
      v-if="recommendedInverter"
      class="bg-green-100 text-green-700 p-1 rounded-lg mb-4 md:flex items-center gap-2 hidden"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      ...
      Recommended:
      <strong> {{ recommendedInverter.brand }} {{ recommendedInverter.model }} </strong>
      ({{ recommendedInverter.power }}W) — based on {{ requiredInverterPower.toFixed(0) }}W
      requirement
    </div>

    <div
      v-if="recommendedInverter"
      class="md:hidden bg-green-100 text-green-700 p-3 rounded-lg mb-4 flex flex-col md:flex-row gap-2 break-words"
    >
      ...
      <span>
        <strong>
          {{ recommendedInverter.brand }}
          {{ recommendedInverter.model }}
        </strong>
    ...
    <label class="flex items-center gap-3 mb-2 cursor-pointer mt-4">
      ...
      <div class="relative">
        ...
        <input type="checkbox" v-model="useCustom" class="sr-only peer" />
        ...
        <!-- Circle -->
        <div
          class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-md transform transition-transform peer-checked:translate-x-5"
        ></div>
      </div>

      <!-- TEXT (RIGHT SIDE) -->
      <span class="text-sm font-medium text-gray-700"> Use Custom Inverter </span>
    </label>
    ...
    <div ref="dropdownRef" class="relative mb-4">
      ...
      >
        <div
          ...
          :class="selectedInverter?.model === inv.model ? 'bg-blue-200' : 'hover:bg-blue-100'"
        >
    ...
    <!-- SPECIFICATIONS -->
    <div v-if="showSpecs" class="mt-4 border rounded-lg p-4 bg-gray-50">
    ...
    <!-- EMPTY STATE -->
    <div v-else class="mt-4 p-6 border rounded-lg text-center text-gray-400">
      ...
      <p class="text-sm">Search and select an inverter to view specifications</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
      <div class="bg-gray-100 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-xl font-bold">{{ Number(adjustedPower).toFixed(0) }} W</p>
        </div>
      ...
      <div class="bg-blue-100 p-4 rounded shadow flex items-center justify-between">
        <div>
          ...
          <p class="text-xl font-bold">{{ requiredInverterPower.toFixed(0) }} W</p>
        </div>
    ...
    <div>
      <InverterSearchModal :show="showModal" @close="showModal = false" />
    </div>
  </div>
</template>
